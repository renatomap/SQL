...
-- 03
SELECT  nomeNovela ,ultCap FROM tbNovela WHERE nomeNovela = 'rei do gado';
...
-- 06
SELECT nomeNovela FROM tbNovela WHERE nomeNovela LIKE '%gado'; 
...
SELECT count(1) AS qntNovelas FROM tbNovela WHERE codNovela IN(
  ...
  WHERE pn.codPersonagem = p.codPersonagem AND p.codAtor = a.codAtor AND a.nomeAtor LIKE 'Antonio Fagundes'
); 
...
-- 12 
SELECT nomeNovela AS nome, count(nomeCap) AS qntCap FROM tbCapitulos AS c INNER JOIN tbNovela AS n 
ON c.codNovela = n.codNovela GROUP BY nome;
...
-- 15
SELECT nomePersonagem AS nome, min(idadePersonagem) AS idade FROM tbPersonagem AS P GROUP BY nome HAVING idade < 35;

-- 16 
SELECT a.nomeAtor, a.idade FROM tbAtor AS a INNER JOIN tbPersonagem AS p ON p.codAtor = a.codAtor
WHERE idade = idadePersonagem;
...
-- 20
SELECT nomeAtor, count(nomePersonagem) FROM tbPersonagem AS p INNER JOIN tbAtor AS a
ON p.codAtor = a.codAtor GROUP BY nomePersonagem;
...
-- 21
SELECT count(nomeCap) AS qntCap, nomeNovela AS novela FROM tbNovela AS n INNER JOIN tbCapitulos AS c 
ON c.codNovela = n.codNovela GROUP BY novela HAVING qntCap > 9;  
...
SELECT nomeAtor AS ator, sitFinanceira AS rico FROM tbAtor AS a INNER JOIN tbPersonagem AS p
ON p.codAtor = a.codAtor AND WHERE sitFinanceira = 'classe media alta';

-- incompleto
SELECT count(nomeAtor) AS ator FROM tbAtor WHERE codAtor IN(
  SELECT n.codNovela FROM tbNovela AS n, tbPersonagem AS p, tbAtor AS a, tbNovelaPersonagem AS np 
  WHERE n.codAtor = p.codPersonagem AND p.codAtor = a.codAtor AND a.nomeAtor LIKE 'Antonio Fagundes'
); -- incompleto
